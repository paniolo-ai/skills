...
description: |
  Playwright E2E test conventions for this project — AAA pattern, translation-aware selectors, hydration waits, mock auth, staging DB sessions, two-user flows, debugging, and CI setup. Use when authoring or editing any Playwright spec, test helper, or E2E configuration. Do NOT use for unit tests or hook tests — load vitest-test-best-practices instead.
license: MIT
metadata:
  version: 0.1.0
tags:
...
**Auth skill:**
[/.agents/skills/playwright-test-auth-best-practices/SKILL.md](../paniolo-playwright-test-auth-best-practices/SKILL.md)
...
- Example spec: [example.spec.ts](/.agents/skills/playwright-test-best-practices/example.spec.ts)
...
```bash
npm run test:e2e:dev -- [filename]                                      # basic tests
npm run test:e2e:dev:staging-db:file -- [filename] --project=chromium  # Realtime/RLS tests
npm run lint                                                             # lint check
```

## Skill handoffs

- Unit or hook tests → load `vitest-test-best-practices` or `vitest-test-hook-best-practices`.
- TypeScript errors in spec files → load `paniolo-typescript-best-practices`.
- Auth setup, mock helpers, or session flows → load
  `playwright-test-auth-best-practices`
  ([SKILL](../paniolo-playwright-test-auth-best-practices/SKILL.md) |
  [playwright-auth](references/playwright-auth.md)).

## Do Not

- Do not use for unit tests or hook tests — load `vitest-test-best-practices` instead.
...
- Do not use for auth setup or session flows — load `playwright-test-auth-best-practices` instead.

## References

- Full reference: [playwright-test](references/playwright-test.md)
- Auth reference: [playwright-auth](references/playwright-auth.md)
- Repo-wide rules: rules
- [playwright-test-auth-best-practices/SKILL.md](../paniolo-playwright-test-auth-best-practices/SKILL.md)
